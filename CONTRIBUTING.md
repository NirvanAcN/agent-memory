...
The scripts and tests have **no runtime dependencies** and target **Python 3.7+**.
Lint tooling is optional.

```bash
python3 -m venv .venv
source .venv/bin/activate
python3 -m pip install "ruff>=0.4"
```
...
5. `context-budget.json` templates and validation when budget behavior changes
